...
select distinct months from (select month(transaction_date) months from coffee) dates;
...
-- month-on-month percentage increase or decreace in sales revenue --
...
-- roll over month on month sales revenue--
...
, count(transaction_qty) count_of_sales,
...
-- Revenue by weekend/weekday for month of may --
select round(sum(unit_price * transaction_qty)) as revenue , 
...
-- comparing daily sales with average sales, if daily sale is above or below average daily sales for the month of may--
...
select hour(transaction_time) as clock, count(*)count_of_orders, sum(transaction_qty) order_qty,
 ...
 -- Revenue and number of sales by day of week --
...
end as days_of_week, count(*) sales_count
from coffee
where month(transaction_date) = 5
